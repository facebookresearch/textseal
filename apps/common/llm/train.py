...
"""
Run with:
    python -m apps.common.llm.train --config /path/to/text-seal
"""
...
from apps.common.llm.transformer import (
    LMTransformerArgs,
    LMTransformer,
...
from apps.wmtraining.lingua.stool import StoolArgs, launch_job
    ...
    with ExitStack() as context_stack:
        ...
        while train_state.step < args.steps:
            ...
            if args.eval is not None and every_n_steps(
                ...
                from apps.common.llm.eval import (
                    launch_eval,
                    EVAL_FOLDER_NAME,
                    EvalArgs,
                )
...
def main():
    try:
        ...
        if "config" in cli_args:
            file_cfg = OmegaConf.load(cli_args["config"])
            del cli_args["config"]
        default_cfg = TrainArgs()
        cfg = OmegaConf.merge(default_cfg, cli_args, file_cfg)
        cfg = OmegaConf.to_object(cfg)
    print(cfg)
    train(cfg)
