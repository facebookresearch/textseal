...
"""
salloc --partition=learn --nodes=1 --gpus-per-node=4 --qos=high --time=6:00:00 --job-name=dev_llmwm --cpus-per-gpu=24--mem-per-cpu=8G
cd /path/to/textseal
conda activate lingua

Run with:
    python -m apps.common.llm.generate --ckpt /path/to/data --prompts "Tomas Soucek is" "Sylvestre Rebuffi is" --temperature 0.9 --top_p 0.95
    python -m apps.common.llm.generate \
...
or
    python -m apps.common.llm.generate \
        ...
        python -m apps.common.llm.generate --ckpt /checkpoint/path/to/checkpoint/consolidated
...
from apps.common.utils.config import dataclass_from_dict, cfg_from_cli
from apps.common.watermark.core import WatermarkArgs, score_tokens
from apps.wmtraining.lingua.checkpoint import CONSOLIDATE_NAME
from apps.wmtraining.lingua.tokenizer import Tokenizer, build_tokenizer
from apps.wmtraining.lingua.transformer import (
    Attention,
    causal_mask,
...
from apps.wmtraining.transformer import LMTransformerWM, LMTransformerArgs


def sample_top_p(probs: torch.Tensor, p: float) -> torch.Tensor:
    """
    Samples a token from the probability distribution using nucleus (top-p) sampling.
    Only the smallest set of tokens with cumulative probability >= p are considered.
    """
...
def sample_top_k(probs, k):
    """
    Samples a token from the probability distribution using top-k sampling.
...
def sample_tokens(logits, temperature=0.0, top_p=None, top_k=None):
    """
    Samples tokens from logits using temperature, top-p, or top-k sampling.
    If temperature is 0, uses argmax (greedy decoding).
    """
    shape = logits.shape
...
def pack_prompts(prompts):
    """
    Packs a list of tokenized prompts into a single tensor and returns their lengths.
    """
    res = []
    lengths = []
...
def batch_prompts(prompts, max_elements, lengths=None):
    """
    Batches prompts so that each batch does not exceed max_elements tokens.
    Returns a list of batches, each containing prompts.
    """
    batches = []
    current_batch = []
    current_count = 0
...
class KVCache(nn.Module):
    """
    Key-Value cache for transformer attention. Stores keys and values for fast autoregressive generation.
    """
...
class PackedCausalTransformerGeneratorArgs:
    """
    Arguments for PackedCausalTransformerGenerator controlling generation parameters and device settings.
    """
...
class PackedCausalTransformerGenerator:
    """
    Efficient generator for causal transformer models that packs multiple prompts together for fast batched generation.
    Handles attention masks, KV cache, and generation logic for multiple sequences.
    """
    def __init__(
        self,
    ...
    def clear_cache(self, offset):
        """
        Clears and resets the KV cache for all Attention modules in the model.
        """
    ...
    def setup_prefilling(self, lengths: torch.Tensor):
        """
        Prepares KV cache, attention masks, and token/document ids for prefilling packed prompts.
        """
    ...
    def setup_generation(self, lengths):
        """
        Sets up KV cache offset and token/document ids for autoregressive generation after prefilling.
        """
    ...
    def prefill(self, tokens: torch.Tensor, lengths: torch.Tensor):
        """
        Prefills the KV cache for packed prompts using block diagonal attention masks.
        Returns model output logits for the prompt tokens.
        """
    ...
    def generate_next_token(self, current_token):
        """
        Generates the next token for each sequence in the batch using the current KV cache and attention mask.
        """
    ...
    def generate(self, prompts, return_aux=False):
        """
        Generates text for a list of prompts using packed batched generation.
        Returns generated text, loglikelihoods, and greedy match info.
        If return_aux is True, also returns auxiliary logits and chosen tokens.
        """
        # Tokenize
        prompts = [
        ...
        for batch in it:
            ...
            # Collect auxiliary info for this batch
            if return_aux:
                aux_logits.append(prompt_logits.detach().cpu())
                aux_chosen_tokens.append(start_token.detach().cpu())
            ...
            for i in range(1, self.max_gen_len):
                ...
                if return_aux:
                    aux_logits.append(next_logits.detach().cpu())
                    aux_chosen_tokens.append(next_token.detach().cpu())
        ...
        if return_aux:
            aux = {
                'logits': torch.cat(aux_logits, dim=1),
                'chosen_tokens':torch.cat(aux_chosen_tokens, dim=1),
                'prompt_tokens': packed_batch.cpu(),
            }
            return generation, loglikelihood, greedy, aux
        return generation, loglikelihood, greedy


def load_consolidated_model_and_tokenizer(
    consolidated_path,
    model_cls=LMTransformerWM,
    model_args_cls=LMTransformerArgs,
):
    """
    Loads a consolidated model checkpoint and tokenizer from disk.
...
def main():
    ...
    model, tokenizer, config = load_consolidated_model_and_tokenizer(cfg.ckpt)
    generator = PackedCausalTransformerGenerator(gen_cfg, model, tokenizer)

    # Load prompts from CLI or interactively
    ...
    for gen in generation:
        tokens = tokenizer.encode(gen, add_bos=False, add_eos=False)
        wm_mask, prop_pred_watermarked = score_tokens(tokens, wm_args)
        wm_pred_props.append(prop_pred_watermarked)
        wm_masks.append(wm_mask)

    # Display the results
    for i, gen in enumerate(generation):
        print(f"\nPrompt {i+1}: {prompts[i]}")
        print(f"Generated Text: {gen}")
        print(f"Proportion watermarked: {wm_pred_props[i]:.4f}")
        print(f"Tokens watermarked: {sum(wm_masks[i])}/{len(wm_masks[i])}")
