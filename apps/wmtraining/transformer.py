...
from apps.wmtraining.wmloss import wm_cross_entropy, sft_wm_cross_entropy
...
class LMTransformerWM(BaseTransformer):
    ...
    def forward(
        ...
        mask: BlockMask | AttentionBias | torch.Tensor | str | None = None,
        attn_impl: str = "sdpa",
        watermark: WatermarkConfig | None = None,
        qa_mask: torch.Tensor | None = None,  # New parameter for SFT
        sft_mode: bool = False,  # Flag to enable SFT mode
    ):
        bsz, seqlen = token_values.shape # b s
        ...
        logits = self.output(self.norm(h)) # b s v
        if target is not None:
            if sft_mode and qa_mask is not None:
                # Use SFT-specific loss function with Q&A masking
                return sft_wm_cross_entropy(
                    logits, 
                    token_values, 
                    target,
                    batch_sources,  # These are now author IDs
                    qa_mask,
                    watermark
                )
            else:
                # Use original pretraining loss function
                return wm_cross_entropy(
                    logits, 
                    token_values, 
                    target,
                    batch_sources,
